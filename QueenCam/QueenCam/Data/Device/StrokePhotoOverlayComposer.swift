//
//  StrokePhotoOverlayComposer.swift
//  QueenCam
//
...
final class StrokePhotoOverlayComposer {
  ...
  private static let stateQueueLabel = "com.queendom.QueenCam.strokePhotoOverlayComposer"
  ...
  private let stateQueue = DispatchQueue(label: stateQueueLabel)
  private var strokeLayers: [StrokeLayerKey: [OverlayStroke]] = [:]

  func replaceVisibleStrokes(_ strokes: [Stroke], for key: StrokeLayerKey) {
    ...
    stateQueue.sync {
      strokeLayers[key] = overlayStrokes
    }
  }

  func clear(_ key: StrokeLayerKey) {
    stateQueue.sync {
      strokeLayers[key] = []
    }
  }
  ...
  private func makeCompositeImage(from imageData: Data) -> UIImage? {
    ...
    let overlayRect = StrokeOverlayRenderer.aspectFitRect(
      aspectRatio: StrokeOverlayRenderer.logicalCanvasAspectRatio,
      in: imageRect
    )
    let lineScale = StrokeOverlayRenderer.lineScale(for: overlayRect)
