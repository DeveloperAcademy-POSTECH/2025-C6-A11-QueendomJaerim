...
- Wi-Fi Aware 페어링 기기의 발견 및 최근 연결 이력을 저장하는 기기 레지스트리 추가

### Changed

- 설정 화면의 펜 가이드 저장 옵션에 촬영자 설정 기준 안내 문구 추가
- 페어링 기기 목록을 신규 기기 우선, 이후 최근 연결 순으로 정렬
- 기기 저장소는 원자적 조회·upsert만 담당하고 기기 이력 유즈케이스는 레지스트리에서 구성하도록 계층 책임 분리
...
- 페어링 기기 저장 재시도 성공 시 실제 저장 기록을 목록에 반영하도록 수정
- 연결 화면을 다시 열어도 페어링 기기 목록 스트림 구독이 유지되도록 수정
